...
function buildOverview(collection: PostmanCollection, endpoints: EndpointDoc[]): DocsOverview {
...
export async function loadDocumentation(): Promise<{ endpoints: EndpointDoc[]; overview: DocsOverview }> {
  ...
  if (!response.ok) {
    throw new Error(`Не удалось загрузить документацию: HTTP ${response.status}`);
  }

  const collection = (await response.json()) as PostmanCollection;
  const endpoints = walkItems(collection.item ?? []);
  const overview = buildOverview(collection, endpoints);
