# Snippet для samotpravil.ru/get-access

Короткий блок для страницы получения доступа или раздела «Интеграция».

---

## MCP для разработчиков

Подключите API СамОтправил к **Cursor**, **Claude Desktop** или **VS Code** через MCP-сервер — поиск документации, typed tools, offline-режим.

```bash
npx -y samotpravil-mcp@latest
```

- [Документация и примеры](https://github.com/dkanster/samotpravil-mcp/blob/main/docs/EXAMPLES.md)
- [GitHub](https://github.com/dkanster/samotpravil-mcp)
- [Полная API-документация](https://documentation.samotpravil.ru/)

После получения API-ключа добавьте его в конфиг MCP как `SAMOTPRAVIL_API_KEY`.
